...
while(true){
    ...
    //发送操作日志
    $SendServerLog = $deviceModel->SendServerLog();
    for($j=0;$j<count($SendServerLog);$j++){
        $WebsocketClient->sendData("class=IntellixlUse&method=SendServerLog&params=".$SendServerLog[$j]['params']);
        $receiveData = $WebsocketClient->receiveData();
        $time = json_decode(json_decode($SendServerLog[$j]['params'])->item)->time;
        if(strpos($receiveData, "成功")!=""){
            $deviceModel->updateServerLogStatus($time);
        } 
    }


    //发送播放日志
    $SendPlayerLog = $deviceModel->SendPlayerLog();
    for($i=0;$i<count($SendPlayerLog);$i++){
        $WebsocketClient->sendData("class=IntellixlUse&method=SendPlayLog&params=".$SendPlayerLog[$i]['params']); 
        $receiveData = $WebsocketClient->receiveData();
        $play_start = json_decode(json_decode($SendPlayerLog[$i]['params'])->item)->play_start;
        if(strpos($receiveData, "成功")!=""){
            $deviceModel->updatePlayLogStatus($play_start);
        }
    }
